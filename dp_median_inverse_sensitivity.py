...
class DpMedianInverseSensitivity(DpMedian):
    ...
    def answer(self, data: list[int]) -> int:
        ...
        for i in range(0, mid_idx):
            if data[i] != data[i-1]:
                intervals.append(self.Interval(data[i], data[i+1]-1, -len_tmp))
            len_tmp += 1
        ...
        len_tmp += 1
        # initialize probability for each interval
        for i in range(0, len(intervals)):
            intervals[i].prob = (intervals[i].end-intervals[i].start+1) * \
                math.exp(-intervals[i].len * self.epsilon / 2.0)
